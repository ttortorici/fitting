...
class Debye33(Debye):
    ...
    def set_population(self, peak_num: int, new_population: float, step: int=0, override: int=None):
        if peak_num == 3:
            self.populations3 = new_population
        else:
            if peak_num == 1:
    ...
    def show_fit(self):
        ...
        capacitance, loss = self.fitting_function(
            self.temperature_fit, self.angular_frequency, self.curie_temperature, self.ln_attempt_time,
            ...
            self.populations1, self.populations2, self.populations3, self.peak_width1, self.peak_width2,
        ...
        for ii in range(self.freq_num):
            ...
            axes[1].plot(self.temperature_fit[:, ii], loss[:, ii], self.colors_f[ii], linewidth=1)
        return fig, axes
